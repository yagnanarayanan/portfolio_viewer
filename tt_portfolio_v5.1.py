...
# fetch the unit prices from a collection where units of the MFs is present
client = pymongo.MongoClient("mongodb://localhost:27017")
db = client["finance"]
collection = db["mf"]
data = collection.find({},{"Scheme Details":1,"Units":1,"_id":0})
scheme_names = []
mf_unit_quantities = []
for document in data:
    scheme_names.append(document["Scheme Details"])
    mf_unit_quantities.append(document["Units"])
total = 0
mf_individual_value = []
# with open("portfolio_val_test.txt", "a", encoding="utf-8") as f:
#     f.write(f"Portfolio as of {datetime.datetime.now()}:\n")
collection_processed = db["mf_processed"]
for i in range(0, 4):
    mf_val = mf_unit_quantities[i] * mf_unit_prices[i]
    mf_individual_value.append(mf_val)
    collection_processed.insert_one({"funds":{"Scheme Details":scheme_names[i],"Units":mf_unit_quantities[i],
                                     "nav":mf_unit_prices[i],"mf_val":mf_val,"date_time":str(datetime.datetime.now())}})
...
db["mf_processed"].insert_one({"funds":{"total_valuation": total, "date_time":str(datetime.datetime.now())}})
# with open("portfolio_val_test.txt", "a", encoding="utf-8") as f:
